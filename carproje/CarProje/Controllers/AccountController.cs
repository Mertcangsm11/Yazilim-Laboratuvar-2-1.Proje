...
namespace CarProje.Controllers
{
        ...
        
    {
        ...
        public ActionResult Login(string username, string password, bool? remember)
        {
            ...
            try
            {
                string q_sel = "UPDATE carcontrol.dbo.USERS  SET LOGIN = GETDATE() WHERE EMAIL ='"+username+"'";
            ...
            return RedirectToAction("Index","Home");
                ...
                if (sayi >= 3)
                {
                    return RedirectToAction("Index", "Account", new { error = "3 veya Daha Fazla Hatalı Giriş Yaptınız, Lütfen Tekrar Deneyiniz..." });
                }
                else
        ...
        [AllowAnonymous]
        public ActionResult Logout(string username, string password, bool? remember)
        {
            string str_connection = ConfigurationManager.ConnectionStrings["conn"].ConnectionString;
            SqlConnection myconn = new SqlConnection(str_connection);
                try
                {
                    string q_sel = "UPDATE carcontrol.dbo.USERS  SET [EXIT] = GETDATE() WHERE EMAIL ='s'";
                    SqlCommand sql_cmd = new SqlCommand(q_sel, myconn);
                    myconn.Open();
                    SqlDataReader dr = sql_cmd.ExecuteReader();
                    dr.Close();
        ...
        public ActionResult Userlist()
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    user.LOGIN = dr[5].ToString();
                    user.EXIT = dr[6].ToString();
                    userlist.Add(user);
                }
        ...
        public ActionResult Adduser(UserModel model)
        {
            ...
            try
            {
                string q_sel = "Insert Into USERS Values (1, @P1, @P2, @P3,'0','0')";
                SqlCommand sql_cmd = new SqlCommand(q_sel, myconn);
